...
namespace MultiScreenshotCaptureNamespace
{
    ...
    public class MultiScreenshotCapture : EditorWindow
    {
        ...
        private readonly List<CustomResolution> queuedScreenshots = new();
        ...
        private List<Vector2> resolutions = new() { new(1024, 768) }; // Not readonly to support serialization
        private List<bool> resolutionsEnabled = new() { true }; // Same as above
        ...
        private void Awake()
        {
            ...
            {
                var sessionData = JsonUtility.FromJson<SessionData>(File.ReadAllText(SESSION_DATA_PATH));
        ...
        private void CaptureQueuedScreenshots()
        {
            ...
            var resolution = queuedScreenshots[0];
        ...
        {
            var camera = targetCamera == TargetCamera.GameView ? Camera.main : SceneView.lastActiveSceneView.camera;
            ...
            try
            {
                ...
                if (SystemInfo.graphicsUVStartsAtTop)
                {
                    ...
                    for (int i = 0; i < height / 2; i++)
                    {
                        ...
                        {
                            var color = pixels[startIndex0 + x];
        ...
        private void LoadSettings()
        {
            ...
            {
                var saveData = JsonUtility.FromJson<SaveData>(File.ReadAllText(loadPath));
